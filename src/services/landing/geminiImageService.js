...
const generateTryOnImage = async (apiKey, base64Image, imageMimeType, product, view = 'front', referenceImage = null) => {
  try {
    const ai = new GoogleGenAI({ apiKey });
    const model = 'gemini-2.5-flash-image';
    const userImagePart = fileToGenerativePart(base64Image, imageMimeType);

    const basePrompt = `Eres un AI experto en moda para la marca colombiana de fajas 'Esbelta'. Toma la imagen de la persona y vístela de manera realista con la faja '${product.name}' color ${product.color}, que es '${product.description}'. ${referenceImage ? 'USA LA IMAGEN DE REFERENCIA DEL PRODUCTO para replicar exactamente el diseño, textura, color y detalles de la faja.' : ''} La persona y la faja deben verse fotorealistas. El fondo debe ser un estudio profesional con fondo gris claro neutro. Superpón una marca de agua semi-transparente con el patrón 'esbelta' repetido sutilmente en todo el fondo. Mantén la etnia y características originales de la persona.`;
    ...
    // Construir array de partes: imagen del usuario + prompt + imagen de referencia (si existe)
    const parts = [userImagePart, textPart];
...
export const generateTryOnImages = async (apiKey, base64Image, imageMimeType, product) => {
  ...
  if (product.referenceImageUrl) {
    try {
      const response = await fetch(product.referenceImageUrl);
      const blob = await response.blob();
      const base64 = await blobToBase64(blob);
      referenceImagePart = fileToGenerativePart(base64, blob.type);
      console.log('Imagen de referencia cargada:', product.referenceImageUrl);
    } catch (error) {
      console.error('Error cargando imagen de referencia:', error);
      // Continuar sin imagen de referencia si falla
    }
  }

  const [frontImage, sideImage] = await Promise.all([
    generateTryOnImage(apiKey, base64Image, imageMimeType, product, 'front', referenceImagePart),
    generateTryOnImage(apiKey, base64Image, imageMimeType, product, 'side', referenceImagePart),
  ]);
